...
public class MainScreen implements Initializable {
	...
	@FXML
	private Button rentBikeButton, returnBikeButton, viewDockListButton, viewRentalBikeButton, viewAccountButton;
	...
	public void handleAccountClick() {
		try {
			FXMLLoader loader = new FXMLLoader(getClass().getResource(Constants.ACCOUNT));
			Parent root = (Parent) loader.load();
			Stage stage = new Stage();
			stage.initModality(Modality.APPLICATION_MODAL);
			stage.setScene(new Scene(root));
			stage.setTitle("AccountScreen");
			stage.show();
	
			Stage oldStage = (Stage) viewAccountButton.getScene().getWindow();
			oldStage.close();
		} catch (IOException e) {
			e.printStackTrace();
		}
